...
# Quantiles of chi-square
lower_quant = qchisq(a1, df)
upper_quant = qchisq(1-a2, df)


# Pivotal CI
theta_lower <- lower_quant / (2*s)
theta_upper <- upper_quant / (2*s)

theta_lower2 <- qgamma(a1, shape = n, rate = s)
theta_upper2 <- qgamma(1-a2, shape = n, rate = s)

CI_pivotal <- c(theta_lower, theta_upper)
cat("95% CI using pivotal:", CI_pivotal, "\n")
...
CI_bayes <- c(theta_bayes_lower, theta_bayes_upper)

cat("95% CI using Bayesian:", CI_bayes, "\n")
